...
function SidebarBody({ onNavigate }: { onNavigate?: (() => void) | undefined }) {
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar p-4 text-sidebar-foreground">
      <Link to="/dashboard" onClick={onNavigate} className="mb-6 flex items-center gap-2 px-1">
        <span className="grid size-9 place-items-center rounded-xl bg-sidebar-primary text-sidebar-primary-foreground">
          <GraduationCap className="size-5" />
        </span>
        <span className="font-display text-base font-bold">EduFlow AI</span>
      </Link>

      <div className="mb-6 rounded-xl border border-sidebar-border bg-sidebar-accent/50 p-3">
        <p className="truncate text-sm font-semibold">{school?.name ?? "Your school"}</p>
        <p className="mt-0.5 text-xs text-sidebar-foreground/60">
          {school?.is_demo ? "Demo workspace" : "School workspace"}
        </p>
      </div>
...
export function AppShell({
  ...
  useEffect(() => {
    if (!loading && profile && !profile.school_id) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border/60 bg-background/85 px-4 py-3 backdrop-blur sm:px-6">
