...
function AiSettingsPage() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const [settingsResult, usageResult] = await Promise.all([
        supabase.from("ai_settings").select("school_id,enabled,student_tutor_enabled,teacher_assistant_enabled,monthly_token_limit,max_output_tokens,disclosure_text").eq("school_id", profile.school_id).single(),
        supabase.from("ai_monthly_usage").select("actual_tokens,reserved_tokens,request_count,total_cost").eq("school_id", profile.school_id).order("usage_month", { ascending: false }).limit(1).maybeSingle(),
      ]);
  ...
  return <AppShell title="AI Settings" description="Control EduFlow AI availability, safety and school-wide usage">
