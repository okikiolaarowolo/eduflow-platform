...
type UsageStatus = { enabled: boolean; plan_token_limit: number | null; used_tokens: number };
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: conversation } = await supabase.from("ai_conversations").select("id,school_id,user_id,mode").eq("id", conversationId).eq("user_id", userData.user.id).single();
    ...
    const { data: roles } = await supabase.from("user_roles").select("role").eq("user_id", userData.user.id).eq("school_id", conversation.school_id);
    const roleSet = new Set((roles ?? []).map((r) => r.role));
    const isTeacher = roleSet.has("teacher"); const isStudent = roleSet.has("student");
    ...
    const usage = usageStatus[0] as UsageStatus;
    if (!usage.enabled) return json({ error: "EduFlow AI is disabled for this school." }, 403);
    const schoolLimit = Math.max(0, Number(settings?.monthly_token_limit ?? 0));
    const planLimit = Math.max(0, Number(usage.plan_token_limit ?? 0));
    const limits = [schoolLimit, planLimit].filter((n) => n > 0);
    const monthlyLimit = limits.length ? Math.min(...limits) : 0;
    ...
    if (subjectId) { const { data: subject } = await supabase.from("subjects").select("name").eq("id", subjectId).eq("school_id", conversation.school_id).maybeSingle(); if (subject?.name) subjectName = subject.name; }
    ...
    const priorMessages = (history ?? []).filter((m: { role: string }) => m.role === "user" || m.role === "assistant").map((m: { role: string; content: string }) => ({ role: m.role as "user" | "assistant", content: m.content }));
    const lastMessage = priorMessages[priorMessages.length - 1];
    const messages = [{ role: "system" as const, content: system }, ...priorMessages, ...(lastMessage?.role === "user" && lastMessage.content === message ? [] : [{ role: "user" as const, content: message }])];
    ...
    const inputTokens = Number(result.usage?.prompt_tokens ?? 0); const outputTokens = Number(result.usage?.completion_tokens ?? 0); const totalTokens = Number(result.usage?.total_tokens ?? inputTokens + outputTokens);
